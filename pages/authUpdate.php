...
// Requette dans la table revenu imposable pour authentifier
    ...
    $rowReq = $revenuStatement->fetch();
     ...
     if($code == $rowReq ['code'] && $nom == $rowReq ['nom'] ){
        $_SESSION['code'] = $code;
        header("Location: update_impot.php?code=". $code);
     }else{
        header("Location: authUpdate.php?errAuth=😥Donnees sont introuvables...");
     }
                    ...
                    <div id=formContent class="col-5">
                       <?php if(isset($_GET['errAuth'])){ ?>
                        ...
                        <form action="#" method="post">
                            <div class="mb-4">
                            <h5> <span>Authentifier</span> | Modifier <i class="bi bi-pencil-square"></i></h5>
                                <i class="bi bi-person iconUser"></i><input class="form-control" name="nomAuth" type="text" placeholder=" Entrer le nom du contribuable" required>
                            </div>
                            <div class="mb-4">
                                <i class="bi bi-lock iconUser"></i> <input class="form-control" name="codeAuth" type="password" placeholder=" Entrer le code fiscal" required>
                            </div>
                            <div class="mb-4">
